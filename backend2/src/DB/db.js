...
const getClient = (url) => new Client(url);
...
export async function insertUser(input, url) {
    const client = getClient(url);
    await client.connect();
    try {
...
export async function insertPost(input, url) {
    const client = getClient(url);
    await client.connect();
    const { title, content, authorId, publishedDate, category_id } = input;
    try {
        const res = await client.query(`INSERT INTO posts(title, content, authorId, publishedDate ,category_id)
       VALUES ($1, $2, $3, $4 ,$5) RETURNING *;`, [title, content, authorId, publishedDate, category_id]);
        return res.rows;
    }
    catch (err) {
...
export async function updatePost(input, url) {
    const client = getClient(url);
    await client.connect();
...
export async function getPostByUserId(id, url) {
    const client = getClient(url);
    try {
        await client.connect();
        const res = await client.query(`
...
export async function findUser(email, url) {
    const client = getClient(url);
    try {
        await client.connect();
...
export async function getUserByUSerId(id, url) {
    const client = getClient(url);
    try {
        await client.connect();
...
export async function getPostByPostId(id, url) {
    const client = getClient(url);
    try {
        await client.connect();
        const res = await client.query(`SELECT posts.id ,posts.title , posts.content,posts.authorid , users.email , users.name FROM posts
      JOIN users ON posts.authorid = users.id
      WHERE posts.id =$1`, [id]);
        console.log(res);
        return res.rows;
...
export async function createcomment(blogId, authorId, content, url) {
    const client = getClient(url);
    try {
        await client.connect();
        const res = await client.query(`INSERT INTO comment(content, authorId, blogId, publishedDate)
       VALUES ($1, $2, $3, NOW()) RETURNING *;`, [content, authorId, blogId]);
        return res.rows[0];
    }
    catch (e) {
...
export async function createrating(blogId, authorId, score, url) {
    const client = getClient(url);
    try {
        await client.connect();
        const res = await client.query(`INSERT INTO rating (score, authorId, blogId)
       VALUES ($1, $2, $3)
       ON CONFLICT (authorId, blogId) 
       DO UPDATE SET score = EXCLUDED.score
       RETURNING *;`, [score, authorId, blogId]);
        return res.rows[0];
    }
    catch (e) {
...
export async function getCommentByPostId(blogId, url) {
    const client = getClient(url);
    try {
        await client.connect();
        const res = await client.query(`
...
export async function getRatingByPostId(blogId, url) {
    const client = getClient(url);
    try {
        await client.connect();
        const res = await client.query(`
...
export async function GetPostByCategories(url, category) {
    const client = getClient(url);
    try {
        await client.connect();
        const res = await client.query(`
